...
echo 'Server = http://192.168.0.123:8080' > /etc/pacman.d/mirrorlist
echo 'Server = http://mirror.xtom.com.hk/archlinux/$repo/os/$arch' >> /etc/pacman.d/mirrorlist
mount "$PARTITION" /mnt
pacman -Syy
# would recommend to use linux-lts kernel if you are running a server environment, otherwise just use "linux"
pacstrap /mnt base base-devel grub openssh sudo vim
